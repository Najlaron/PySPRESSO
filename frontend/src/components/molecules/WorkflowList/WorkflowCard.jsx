...
// vizualizace již existujícího workflow v seznamu
    ...
    return (
        <div ref={containerRef} className={`relative shadow-md p-ds-lg rounded-xl w-150 transition duration-300 hover:shadow-xl hover:-translate-y-2 cursor-pointer ${isSelected ? "bg-espresso" : "bg-light-foam"}`} onClick={onClick}>
            ...
            <div className="flex items-center justify-between mb-ds-md ">
                ...
                {/* menu s akcema, jako u karty metody */}
                <button
                    onClick={(e) => {
                        e.stopPropagation();
                        setMenuOpen((s) => {
                ...
                {menuOpen && (
                    <div className="absolute right-10 top-15 w-45 bg-foam border border-roast/20 rounded shadow-md z-20 p-1">
                        <button
                        ...
                        >
                            ...
                            <span className="text-base text-noir">Delete workflow</span>
                        </button>
                    </div>
                )}
            </div>
