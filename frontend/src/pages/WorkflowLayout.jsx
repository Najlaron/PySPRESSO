...
function WorkflowLayout() {
    ...
    // kurzor točícího se kolečka v případě vykonávání akce
    // (přidávání metody, spouštění metody/metod, mazání metody)
    useEffect(() => {
        if (isBusy) {
    ...
    // při změně ID v url načte nové workflow a všechny metody
    useEffect(() => {
        // načte workflow podle ID
    ...
    // kategorie, jejichž metody nelze zařadit do workflow
    const blockedCategories = new Set(["io", "initialization"])

    // všechny kategorie metod, použito pro filtrování mezi metodami při přidávání
    const categories = Array.from(
        new Set(
            operations
                .flatMap((op) => op?.categoryTags ?? [])
                .map((tag) => String(tag).trim())
                .filter(Boolean)
                .filter((tag) => !blockedCategories.has(tag.toLowerCase()))
        )
    ...
    // pokud se zrovna nastavují parametry, zavolá se, aby nedošlo k přerušení nastavování
    ...
    // metoda pro přidání kroku do workflow
    async function handleAddStep(operation) {
        await parameterSubmitPromiseRef.current // čeká se až se nastaví parametry
    ...
    // metoda pro vykonání všech kroků
    ...
    // metoda pro smazání kroku z workflow
    ...
    // zavře formulář s parametry (po nastavení)
