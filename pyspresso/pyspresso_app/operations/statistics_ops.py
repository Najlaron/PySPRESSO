...
def statistics_correlation_means(
    ...
    name = os.path.join(
        statistics_folder,
        output_file_prefix + "_group_correlation_matrix_heatmap_" + str(plt_name_suffix),
    )
...
def statistics_PCA(state: WorkflowState, n_components_for_candidates=2):
    ...
    candidates_path = _unique_path(
        os.path.join(statistics_folder, output_file_prefix + "_candidates_after_PCA.csv")
    )
    ...
    _add_artifact_if_available(state, candidates_path, "table", "Candidate features after PCA")
...
@register_operation(
    ...
    requires=["data", "metadata"],
    produces=["plsda_model", "plsda_stats", "plsda_metadata", "plsda_vip_scores", "candidates"],
)
def statistics_PLSDA(
    ...
    vip_path = _unique_path(os.path.join(statistics_folder, output_file_prefix + "_PLSDA_VIP_scores.csv"))
    stats_path = _unique_path(os.path.join(statistics_folder, output_file_prefix + "_PLSDA_stats.csv"))
    candidates_path = _unique_path(os.path.join(statistics_folder, output_file_prefix + "_candidates_after_PLSDA.csv"))
    ...
    _add_artifact_if_available(state, candidates_path, "table", "Candidate features after PLS-DA")
...
def statistics_ttest(
    ...
    _add_artifact_if_available(state, csv_name, "table", "t-test / Mann-Whitney results")

    if report is not None:
        text0 = f"<b>t-test</b> was performed for the groups: {group1} vs {group2}."
        text1 = "The results are shown in the table below."
        report.add_together([("text", text0), ("text", text1), ("table", p_values_table), "line"])
...
def visualizer_PCA_grouped(
    ...
    if crossout_outliers:
        ...
        outliers = pca_df[
            (np.abs(pca_df["PC1_zscore"]) > 3)
            | (np.abs(pca_df["PC2_zscore"]) > 3)
        ]
    ...
    for combination in existing_combinations:
        ...
        marker = (
            ...
            else class_type_markers.get(class_type, "o")
        )
